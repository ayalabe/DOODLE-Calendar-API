...
public class MessagePushService {
	...
	public byte[] encryptMessage(User user, Object message) throws InvalidKeyException, JsonProcessingException, NoSuchAlgorithmException, InvalidKeySpecException, InvalidAlgorithmParameterException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException { 
		byte[] result = cryptoService.encrypt(objectMapper.writeValueAsString(message),user.getKeys(),user.getAuth(), 0);
		return result;
	}
	...
	public boolean sendPushMessage(User user, byte[] body) {
		String origin = null;
		try {
			URL url = new URL(user.getEndPointLog());
		...
		URI endpointURI = URI.create(user.getEndPointLog());
		...
		try {
			...
			case 201:
				Application.logger.info("Push message successfully sent: {}", user.getEndPointLog());
				break;
			case 404:
			case 410:
				Application.logger.warn("Subscription not found or gone: {}", user.getEndPointLog());
